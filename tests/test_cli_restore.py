...
def test_restore_happy_path_exit_0(capsys: pytest.CaptureFixture[str]) -> None:
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        assert "Stack parado correctamente" in captured.out
        assert "snapshot, extracción, rsync" in captured.out
...
def test_restore_disk_check_path_defaults_to_opencloud_root() -> None:
    with tempfile.TemporaryDirectory() as temporary_directory:
        opencloud_root = _setup_opencloud_root(temporary_directory)
        with (
            patch("opencloud_backup.cli.run_restore_job") as mock_job,
            ...
            main(["restore", "--opencloud-root", str(opencloud_root)])
        assert mock_job.call_args.kwargs["disk_check_path"] == opencloud_root.resolve()
